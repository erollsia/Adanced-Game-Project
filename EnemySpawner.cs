...
namespace Gamekit2D
{
    public class EnemySpawner : ObjectPool<EnemySpawner, Enemy, Vector2>, IDataPersister
    {
        ...
        void Start()
        {
            ...
            {
                Pop(transform.position + transform.right * Random.Range(-spawnArea * 0.4f, spawnArea * 0.4f));
            }
        ...
        protected IEnumerator SpawnTimer()
        {
            while (m_CurrentSpawnedEnemyCount < concurrentEnemiesToBeSpawned && m_TotalSpawnedEnemyCount < totalEnemiesToBeSpawned)
            {
                ...
                Pop(transform.position + transform.right * Random.Range(-spawnArea * 0.6f, spawnArea * 0.6f));
                m_CurrentSpawnedEnemyCount++;
        ...
        {
            Gizmos.DrawWireCube(transform.position, new Vector3(spawnArea, 0.2f, 1));
        }
    }
}
